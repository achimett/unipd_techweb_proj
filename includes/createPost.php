...
function createPost($db) {
  ...
  if (isset($_GET['id']) === true) {
    ...
    if ($post['chiuso'] === true && $_SESSION['user_id'] !== $post['id_autore']) {
      ...
      $result .= '<form action="doit()" id="doit"><fieldset><legend class="post_titolo">' . $legend . '</legend><input type="button" name="DOIT" value="' . $pulsante . '" tabindex="21" /></fieldset></form><div id="post_core">';
    }
    ...
    for($i = 0; $i < count($volontari); $i++) {
      $result .= '<li class="post_listavolontario"><a href="/unipd_techweb_proj/profilo.php?id=' . $volontari[$i]['id'] . '" class="usr"><img alt="' . $volontari[$i]['nome'] . '" src="' . $volontari[$i]['img_path'] . '" />' . $volontari[$i]['nome'] . '</a></li>';
    }
    ...
    for($i = 0; $i < count($commenti); $i++) {
      ...
      <a href="/unipd_techweb_proj/profilo.php?user_id=' . $commenti[$i]['user_id'] . '" class="usr autore_commento"><img alt="' . $commenti[$i]['nome'] . ' ' . $commenti[$i]['cognome'] . '" src="' . $commenti[$i]['img_usr_path'] . '" />' . $commenti[$i]['nome'] . ' ' . $commenti[$i]['cognome'] . '<span>' . $commenti[$i]['data'] . '</span></a>
